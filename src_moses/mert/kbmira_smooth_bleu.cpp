...
int main(int argc, char** argv)
{
  ...
  cerr << "kbmira for smoothed BLEU with c=" << c << " no_shuffle=" << no_shuffle << endl;
    ...
    for(train->reset(); !train->finished(); train->next()) {
      ...
      for(size_t safe_loop=0; safe_loop<2; safe_loop++) {
        ...
        for(size_t i=0; i< train->cur_size(); i++) {
          ...
          ValType score = wv.score(vec);
          ValType bleu = smoothedSentenceBleu(train->scoresAt(i), bleuSmoothing, smoothBP);
          // Hope
      ...
      if(hope_index!=fear_index) {
        ...
        ValType hopeBleu = smoothedSentenceBleu(hope_stats, bleuSmoothing, smoothBP);
        const vector<float>& fear_stats = train->scoresAt(fear_index);
        ValType fearBleu = smoothedSentenceBleu(fear_stats, bleuSmoothing, smoothBP);
