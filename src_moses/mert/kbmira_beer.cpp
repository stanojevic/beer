...
// vim:tabstop=2
/***********************************************************************
 * COMMIT ab06edda5b5ce64d6f624aca234146e6d222d407
...
ValType evaluate(HypPackEnumerator* train, const AvgWeightVector& wv)
{
  ...
  return beerScore(stats);
}

int main(int argc, char** argv)
{
  const ValType BEER_RATIO = 5;
  bool help;
  ...
  bool safe_hope = false; // Model score cannot have more than BEER_RATIO times more influence than BEER
  ...
  cerr << "kbmira for BEER with c=" << c << " no_shuffle=" << no_shuffle << endl;
  ...
  cerr << "Initial BEER = " << evaluate(train.get(), wv.avg()) << endl;
  ValType bestBeer = 0;
    ...
    for(train->reset(); !train->finished(); train->next()) {
      ...
      for(size_t safe_loop=0; safe_loop<2; safe_loop++) {
        ...
        for(size_t i=0; i< train->cur_size(); i++) {
          ...
          ValType score = wv.score(vec);
          ValType bleu = beerScore(train->scoresAt(i));
          // Hope
        ...
        // where model score is having far more influence than BEER
        hope_bleu *= BEER_RATIO; // We only care about cases where model has MUCH more influence than BEER
      ...
      if(hope_index!=fear_index) {
        ...
        ValType hopeBeer = beerScore(hope_stats);
        const vector<float>& fear_stats = train->scoresAt(fear_index);
        ValType fearBeer = beerScore(fear_stats);
        assert(hopeBeer + 1e-8 >= fearBeer);
        ValType delta = hopeBeer - fearBeer;
        // Loss and update
        ...
        if(verbose) {
          ...
          cerr << "Wght: " << wv << endl;
          cerr << "Hope: " << hope << " BEER:" << hopeBeer << " Score:" << wv.score(hope) << endl;
          cerr << "Fear: " << fear << " BEER:" << fearBeer << " Score:" << wv.score(fear) << endl;
          cerr << "Diff: " << diff << " BEER:" << delta << " Score:" << diff_score << endl;
