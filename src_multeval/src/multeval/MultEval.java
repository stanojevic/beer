...
public class MultEval {
	...
	public static Map<String, Metric<?>> KNOWN_METRICS = ImmutableMap.<String, Metric<?>> builder()
			...
			.put("beer", new multeval.metrics.BEER())
			.put("ter", new TER())
    ...
    private static String getMultEvalHome(){
		Class<?> clazz = null;
		try {
			clazz = Class.forName("multeval.MultEval");
        ...
        private static String loadVersion() throws IOException {
        	String multevalHome = getMultEvalHome();
            Properties props = new Properties();
            FileInputStream in = new FileInputStream(multevalHome+"/constants");
            props.load(in);
            in.close();
	...
	public static void main(String[] args) throws ConfigurationException, IOException,
            ...
            System.err.println(String.format("MultEval V%s\n", version) +
                               "By Jonathan Clark\n" +
                               "Using Libraries: METEOR (Michael Denkowski) and TER (Matthew Snover) and BEER (Miloš Stanojević and Khalil Sima'an)\n");
