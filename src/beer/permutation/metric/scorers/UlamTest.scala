...
import beer.permutation.metric.scorers.Ulam.evaluate

class UlamTest extends FlatSpec with ShouldMatchers {
  ...
  "for sort of half score" should "be around 0.75" in {
    val p = List(0, 1, 2, 4, 3)
    val score = evaluate(p)
    
    score should be (0.75 plusOrMinus 0.00001)
  }
