...
private class WMT13 (dir:String) {
  
  val researchers_csv = dir+"/wmt13-researchers-20130630.csv"
  val turkers_csv = dir+"/wmt13-turkers-trimmed.csv"

  val references_dir = dir+"/wmt13-baselines/wmt13-data/plain/references" // newstest2013-ref.$lang
  val system_dir = dir+"/wmt13-baselines/wmt13-data/plain/system-outputs/newstest2013" //  lang pairs dirs
  ...
  def load() : Unit = {
    ...
    load_csv("wmt13", researchers_csv);
    // load_csv("wmt13", turkers_csv);
  }
  ...
  private def load_csv(dataset_name:String, csv_fn:String) : Unit = {
    ...
    for(line <- file_iterator){
      ...
      val src_lang_short = WMT13.long_to_short(src_lang_long)
      val tgt_lang_short = WMT13.long_to_short(tgt_lang_long)
  ...
  private def load_system_translations() : Unit = {
    for(lp <- lang_pairs){
      ...
      for(file <- WMT13.getListOfFiles(system_dir+"/"+lp)){
        val system = file.getName
        system_sents(system) = WMT13.loadContent(file)
      }
  ...
  private def load_references():Unit={
    for(lang <- langs){
      ...
      val content = WMT13.loadContent(new File(fn))
      for(lp <- lang_pairs){
