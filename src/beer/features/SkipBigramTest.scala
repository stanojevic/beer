 ...
 * @author milos
 */
class SkipBigramTest extends FlatSpec with ShouldMatchers {
  ...
  "feature" should "work" in {
    ...
    val fScores = List()
    val gScores = List()
    val f = new SkipBigram(order, symmetry, fScores, gScores)
  ...
  "function" should "anti-symmetric" in {
    val order = 0
    val symmetry = false
    val fScores = List(0.1, 0.2, 2, 3)
    val gScores = List()
    val f = new SkipBigram(order, symmetry, fScores, gScores)
    val sys = "Hello I am ba bam bab am Hello".split(" ")
    val ref = "Hello I am tra lal al alla".split(" ")
    val res1 = f.featureValues(sys, ref)
    println(s"symmetry=$symmetry $res1")
    val res2 = f.featureValues(ref, sys)
    println(s"symmetry=$symmetry $res2")
