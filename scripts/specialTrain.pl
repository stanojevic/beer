...
my @langsToUseForTraining  = ("en", "cs");

my @langPairsAvailableForTraining = ("en-cs", "cs-en", "fr-en", "de-en", "es-en", "ru-en", "hi-en");
...
my %configTemplates = 
	(
	 en => ["func_exact_para_stem_syn", "exact"   ],
	 cs => ["func_exact_para"],
	);
...
sub prepareNbest{
	print STDERR "######## CREACING UNLABELED DATA ###########\n";
	my @trainingDesc = (['cs', "$experiments_dir/../czech.best100.out.gz"    , "$experiments_dir/../czech.reference"   ], 
                        ['en', "$experiments_dir/../english.best100.out.gz"  , "$experiments_dir/../english.reference" ]);

	mkdir $unlabeledTraining;

	for my $lpSet (@trainingDesc){
		my ($lang, $nbest, $ref) = @$lpSet;
		mkdir "$unlabeledTraining/$lang";
		`$BEER_HOME/scripts/extractUnlabeledExamples.pl $nbest $ref 1500000 $unlabeledTraining/$lang`;
	}
}
...
#				$modelType = "tuning_$modelType";
				$cmd = "$BEER_CMD -l $lang --modelType $modelType --workingMode train --trainingWinnerFeaturesFile $winnerFile --trainingLoserFeaturesFile $loserFile --modelDescFile $BEER_HOME/templates/$template".
					" --trainingUnlabeledFeaturesFile1 $unlabeledFile1 --trainingUnlabeledFeaturesFile2 $unlabeledFile2 --unsupervisedIterations 5 >> $algorithm.subProcesses.std 2>> $algorithm.subProcesses.err";
				print STDERR "EXECUTING !!! $cmd !!!\n";
				`$cmd`;
				print STDERR "DONE !!! $cmd !!!\n";
			}else{
				print STDERR "\n\n\n\n\nNOOOOOOOOOOOOOOOOO TUNING MODEL!!!\n\n\n\n\n";
			}
